...
namespace Tetris3D
{
   public class Ai : TetrisScreen
   {
      Board current;
      TetrisSession tetrisSession;
      ...
      {
         get
         { return TetrisModes.Ai; }
      }
       
      public Ai(Microsoft.Xna.Framework.Game game)
         : base(game)
      {
      }
      ...
      {
         if (this.pieceNumber != tetrisSession.pieceNumber)
         {
            this.pieceNumber = tetrisSession.pieceNumber;
            current = new Board(tetrisSession);
            current.boardVariants();
         }
